...
class SumoEnv(gym.Env):
    def __init__(self):
        ...
        # Initialize SUMO
        self.sumo_config_file = "sim5.sumocfg"
    ...
    def apply_action(self, action):
        ...
        for i in range(24): # 24 traffic lights
            if (action[i] == 1):
                phase = 0  # Set traffic light to red (phase 2)

            else:
                phase = 2 # Set traffic light to green (phase 0)
...
class GymWrapper(Wrapper):
    ...
    def reset(self, **kwargs):
        print("Wrapper: Resetting the environment")
        return super().reset(**kwargs)

    def step(self, action):
        print(f"Wrapper: Taking step with action: {action}")
        observation, reward, terminated, truncated, info = super().step(action)
        return observation, reward, terminated, truncated, info

    def close(self):
        print("Wrapper: Closing the environment")
        super().close()
"""
